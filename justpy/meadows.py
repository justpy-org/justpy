    ...
    async def feed(self, html_string):
        ...
        while True:
            ...
            if 'x-data' in attr_dict:
                ...
                # result = (eval(tag.attrs['x-data'], self.context.f_globals, self.for_var))
                if inspect.iscoroutine(result):
                    result = await result
                eval_value = result
            ...
            if 'x-ref' in attr_dict:
                # JustPy.refs[tag.attrs['x-ref']] = c
                wp.meadows_data['refs'][attr_dict['x-ref']['value']] = c

            if ':x-ref' in attr_dict:
                eval_value = await self.async_evaluate_attr(tag, ':x-ref', attr_dict)
                # JustPy.refs[eval_value] = c
                wp.meadows_data['refs'][eval_value] = c

            if 'x-if' in attr_dict:
                if self.show_flag:
                    # JustPy.if_list.append((c, tag.attrs['x-if'], self.context.f_globals))
                    # attr_dict['class']['value']
                    # wp.meadows_data['if'].append((c, tag.attrs['x-if'], self.context.f_globals))
            ...
            if ':class' in attr_dict:
                # JustPy.class_evaluate_list.append((c, tag.attrs[':class'], self.context.f_globals))
                wp.meadows_data['class_evaluate'].append((c, tag.attrs[':class'], self.context.f_globals))
                # class_dict = self.evaluate(tag.attrs[':class'])
...
'''
            ...
            if 'x-for-old' in attr_dict:
                ...
                attr_value = re.sub(' +', ' ', attr_value)  # Remove multiple spaces
                for_var = attr_value.split()[1]
                # for_vars = self.get_for_vars(attr_value)
                init_string = ''
                for k in ['this']:
                    init_string = f'{init_string}{k}=c1.data["{k}"];'

                if self.for_var:
                    init_string = f'{init_string}{self.for_var_name}={self.for_var.__repr__()}'
                # for k in self.for_var:
                #     init_string = f'{init_string}{k}={k.__repr__()};'
                # func = f'async def for_func{JustPy.event_handler_count}(c1, pos=None):\n {init_string}\n {attr_value}'
                func = f'async def {func_name}(wp, c1, pos=None):\n {init_string}\n {attr_value}'
                ...
                func = func + '\n  s = """\n' + for_html_str + '\n"""'
                func = f'{func}\n  c1.data["{for_var}"] = {for_var}'
                func = f'''{func}\n  t = await jp.new_parse_meadows_html(wp, s, eval_flag=True, x_data=c1.data, show_flag=False, for_var={for_var}, for_var_name='{for_var}')'''
                func = f'{func}\n  t._for_creation_{event_handler_count} = True'
                func = f'{func}\n  for child in t.components:'
                func = f'{func}\n   child._for_creation_{event_handler_count} = True'
                func = f'{func}\n   c1.add_component(child, pos)'
