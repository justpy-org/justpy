...
def test_pd():
    ...
    grid = jp.AgGrid(a=wp, classes='m-2 p-2', style='height: 500px; width: 700px', auto_size=True, theme='ag-theme-balham')
    grid.load_pandas_frame(iris)
    for col in grid.options.columnDefs:
        col.editable = True
    grid_stats = jp.AgGrid(a=wp, classes='m-2 p-2', style='height: 500px; width: 700px', auto_size=True, theme='ag-theme-balham')
