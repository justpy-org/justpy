...
Run the following and move the mouse in and out of the element on the page. Click the element also and see what happens.
```python
import justpy as jp
...
def event_demo():
    wp = jp.WebPage()
    d = jp.Div(text='Not clicked yet', classes='w-64 text-2xl m-2 p-2 bg-blue-500 text-white', a=wp,
...
The `components` attribute is a list that holds all the child component instances of the element in the order they will be rendered. It is named "components" because it holds the building blocks of the element.  
...
### Example 3 - Event changing elements on page
