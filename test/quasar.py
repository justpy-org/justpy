...
def tree_test():
    ...
    b1 = QBtn(label='Expand', a=d, click=expand_tree)
    b2 = QBtn(label='Collapse', a=d, click=collapse_tree)
...
class My_expansion(QExpansionItem):

    image_num = 10      # Start from image 10, previous are boring to my taste

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        outer_div = Div(classes="q-pa-md", a=self)
        QImg(src=f'https://picsum.photos/400/300/?image={My_expansion.image_num}', a=outer_div)
        self.icon = "photo"
        self.label = f'Image {My_expansion.image_num}'
        self.value = True
        self.expand_separator = True
        # self.group ="mygroup"    # Makes it accordion
...
def pics(request):
    ...
    close_btn = QButton(label='Close All', classes="q-ml-md q-mb-md", color='negative', a=d)
    open_btn = QButton(label='Open All', classes="q-ml-md q-mb-md", color='positive', a=d)
    l = QList(bordered=True, a=d)
    wp.list = l
