...
def happiness_plot():
    ...
    for i in df.columns[3:]:
        wp.toggle_list.append(jp.QToggle(checked_icon='check', color='green', unchecked_icon='clear', value=True,
                                         label=f'{i}', a=d, style='margin-right: 10px', input=series_change))
    ...
    g.options.columnDefs[0].rowDrag = True
...
def create_corr_page():
    ...
    for x_col in score_factors:
        for y_col in score_factors[score_factors.index(x_col)+1:]:
            x = df[x_col]
            y = df[y_col]
            chart = jp.Scatter(x, y, a=d, style='width: 400px; height: 400px; margin: 10px;', classes='border')
            o = chart.options
